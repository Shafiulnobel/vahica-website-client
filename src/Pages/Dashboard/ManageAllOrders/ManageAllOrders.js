...
const ManageAllOrders = () => {
    ...
    useEffect(()=>{
        fetch('https://nameless-reaches-52059.herokuapp.com/orders')
        .then(res=>res.json())
    ...
    const handleDelete = id =>{
        ...
        if(proceed){
          fetch(`https://nameless-reaches-52059.herokuapp.com/orders/${id}`,{
            method:'DELETE'
          })
          .then(res=>res.json())
          .then(data=>{
      ...
      const handleUpdate = id =>{
        ...
        const url=`https://nameless-reaches-52059.herokuapp.com/orders/${id}`
        fetch(url)
        .then(res=>res.json())
        .then(data=>{
                    ...
                    <Card style={{height:'30rem'}}>
                      ...
                      <Row className="mb-3">
                          <Col className="text-start" xs={12} md={12}>
                              <Card.Title style={{fontWeight:'300',fontSize:'13px'}}>client:
                     ...
                     <Row>
                      ...
                      <Col xs={12} md={6}  className="text-start pt-3">
                       <span>status : <span className="text-warning"> {order.status}</span></span>
                      <Button onClick={()=>handleUpdate(order._id)} className="btn btn-primary">status update</Button>
                      </Col>
                      </Row>
                      </Card.Body>
                    </Card>
